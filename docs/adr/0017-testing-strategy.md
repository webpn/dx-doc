...
The reason to prefer this over a shared dataset is legibility: the precondition of a test is visible inside the test. A test that reads *"given a user with no grant on the project"* and asserts a 403 says what it means, and stays true regardless of what any shared fixture happens to contain this month.
