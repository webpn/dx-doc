...
2. **The API is complete and migration-grade** (REQ-MIG-002 … REQ-MIG-005). Every R1 entity is creatable and updatable through the API; assets upload through it; writes are idempotent upserts keyed on an external reference; batch endpoints make bulk ingestion practical.
...
- **Verification shifts from the importer's report to the Platform's own reconciliation** (REQ-MIG-006). Counts per entity type per project, comparable against the source by a human. This is stronger than an importer's self-report, because it is generated by the system being written to rather than by the process doing the writing.
...
- **The `external_ref` upsert key is a permanent model addition**, not migration scaffolding. It is what makes any future bulk ingestion idempotent, and retrofitting it after write endpoints exist would mean reworking all of them — which is why it lands in R0/R1 rather than when first needed.
...
- Requirements: REQ-MIG-001 … REQ-MIG-009, REQ-API-002, REQ-API-003, REQ-API-004, REQ-API-009, REQ-VER-010
