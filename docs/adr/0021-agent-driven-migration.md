...
2. **The API is complete and import-grade** (REQ-IMP-002 … REQ-IMP-005). Every R1 entity is creatable and updatable through the API; assets upload through it; writes are idempotent upserts keyed on an external reference; batch endpoints make bulk ingestion practical.
