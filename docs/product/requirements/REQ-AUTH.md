...
> **Status on 2026-08-21.** `Property`, `Module`, `Destination`, `NavigationEvent`, `Tracking`, `TrackingProperty` presence, `TrackingTemplate`, `FreePage`, `Flow`, `Trigger`, `Page` and `Project` take `expectedUpdatedAt` and enforce it as an atomically guarded `UPDATE ... WHERE id = ? AND updated_at = ?`, returning `stale_write` when the row changed since the caller's read. **`CompanyService.update` still does a read-compare-write check** — the same race these others used to have — and is tracked as backlog rather than fixed here. `expectedUpdatedAt` is optional per call on every covered entity; omitting it writes unconditionally (last-write-wins), which is the client's choice to make, not the server's. What remains for this requirement: `Company` needs the same atomic guard, and the conflict is not yet surfaced comprehensibly in the UI (still [M1.16](../milestones.md#m116--authoring-ui)).
