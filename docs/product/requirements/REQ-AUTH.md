...
> **Carried forward on 2026-08-18; client surface added on 2026-09-03.** The index is built correctly — properties, trackings, specific values, publishable free pages only — and the leak guards are in place. The shipped Pagefind adapter currently maintains an in-process project-scoped query map, so `GET /api/projects/:id/search?q=...` returns typed results after the same `project.read` grant check; no browser-served Pagefind index route or draft/published selector exists yet. The authenticated project search screen is reachable at `/projects/:id/search` and covered by component/API tests, but this requirement remains **In Progress** pending browser acceptance and the two-index freshness model described by [ADR-0009].
