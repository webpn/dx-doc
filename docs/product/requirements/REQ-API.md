...
The REST API is the foundation of the system, not a feature. The web client, the MCP server, the export generators, the static-site builder and the migration scripts all consume it. All validation lives behind it (REQ-FDN-010).
...
- Write endpoints accept `external_ref` from the outset (REQ-MIG-003). Retrofitting it later would mean reworking every one of them.

> Weak programmatic access was pain point 6. Making the API the only entry point is what prevents it recurring: a capability that exists in the UI cannot fail to exist for machines. Migration (REQ-MIG-002) is the first hard test of that claim, at week 5 rather than R3.
...
**Acceptance**
- The migration script (REQ-MIG-007) is written against the published documentation alone, with no reading of Platform source. If that is not possible, the documentation is incomplete.
...
> **Moved R3 → R1.** An agent cannot write a migration script against an undocumented API, and the alternative — the agent reading the Platform's source — is exactly the coupling that makes the result unmaintainable.

### REQ-API-003 — MCP read tools

**Must** · R1 · [M1.3](../milestones.md) · spec §12.2 · **Not Started** · Issue: — · PR: —

A layer above the REST API. Clients are analysts' and editors' AI assistants, developers' IDEs, and — from M1.4 — the migration agent.

R1 set, chosen for what migration and verification need: list projects; retrieve a project's page structure; retrieve a page's trackings; search properties; retrieve property detail; retrieve the reconciliation report (REQ-MIG-006). The richer analytical tools are REQ-API-010.

**Acceptance**
- Every read tool is scoped by the caller's project grants (REQ-SEC-003), with the negative case tested.
- An agent can verify what it wrote by reading it back through these tools, which is what makes a migration self-checking rather than blind.
...
Write tools covering the R1 entity set (REQ-MIG-002) — not only the narrower list in specification §12.2, which predates migration depending on them.
...
> **Moved R3 → R1.** Migration content is entirely agent-written, and from R1 onward agents and humans share the same draft — which is why attribution in the diff moved to R2 rather than staying in R3.
...
Stays in R3: this is the interactive-assistant path. Migration authenticates with a service-account token (REQ-API-009), which is a narrower and simpler mechanism appropriate to a scripted client.
...
- The migration agent consumes them, so migrated content follows house conventions from the first product rather than being corrected afterwards.

> **Moved R3 → R1**, promoted Could → Should. It was a nice-to-have when agents were a late-release feature; with an agent authoring the migration for ~30 products, guidelines it can actually read are what keep the output consistent.
...
Bulk operations (REQ-AUTH-010) and batch writes (REQ-MIG-005) are exposed through the API and MCP with the same validation, but the target must be an explicit list of identifiers. A filter expression is never accepted as an operation target.
...
The analytical read tools beyond what migration required: retrieve the flow and trigger structure of a project, retrieve the changelog between two versions, impact analysis for a property, and property detail enriched with data-quality status once R4 exists.
