...
| REQ-MIG-007 | Import scripts committed and re-runnable | Must | R1 | M1.4 | Not Started |
| REQ-MIG-008 | Source system frozen, then read-only archive | Must | R1 | M1.10 | Not Started |
...
- Import capability is expressed entirely as general-purpose API and MCP surface (REQ-MIG-002 … REQ-MIG-006), all of which has value beyond any single import.
...
- Every image referenced by imported content resolves after import; the reconciliation report (REQ-MIG-006) lists any that do not.
...
### REQ-MIG-007 — Import scripts committed and re-runnable
...
> The agent's deliverable is the *authoring* of the script; the script performs the import. An agent may quietly coerce unanticipated input into something that looks right — where a parser would fail loudly — so review of the script, the reconciliation counts, and the item-by-item check on the first project are the three mitigations, and none is optional.

### REQ-MIG-008 — Source system frozen, then read-only archive
