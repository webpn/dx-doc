...
> **Found: a freshly created company has no path to its first Admin — decision needed before the M1.15 acceptance path can pass end-to-end (2026-08-20).** Walking the exact exit criterion in [M1.15](../milestones.md#m115--client-foundation) — bootstrap administrator logs in, creates a company, creates a project, grants an editor — deadlocks at "creates a project." `ProjectService.create` requires `company.manage_projects`, gated by `canInCompany`, which requires `user.companyId === companyId`; the instance administrator's `companyId` is permanently `null` by this requirement's own "no implied content access" rule, so they can never pass it for any company, including one they just created. The obvious next step, inviting the first company user, is blocked the same way: `company.invite_user` also requires `canInCompany`, and a brand-new company has no member yet who could hold it. `GrantService.setRole`'s own comment states the same wall from the other side: "an instance admin outside any tenant can never be granted (no company)." This is not a missing route — it is a missing capability: **nothing in the exposed API can create a company's first Admin.** `docs/product/milestones.md` M1.12's line "`ProjectService.create` grants its creator on the project it just created" assumed a creator who already belongs to the company; it does not by itself close this gap.
>
> Candidate resolutions, not yet chosen:
>
> 1. Company creation optionally provisions its first Admin in the same call (email + password, or an invite-style passwordless account), mirroring how `BootstrapService` seeds the instance administrator — smallest change, keeps the instance admin permanently company-less.
> 2. A one-time, audited "assign instance admin as this company's Admin" action, scoped to freshly created companies with zero members — closer to the current model's shape but adds a new capability class to review.
> 3. Relax `canInCompany`/`setRole` to let the instance administrator pass company-scoped checks on a company with no members yet — smallest code diff, but chips at "no implied content access" and needs care that it cannot be used against a company that already has an Admin.
>
> Until one is chosen and implemented, the M1.15 exit criterion cannot be demonstrated end-to-end through the API alone, and the Playwright acceptance test (`e2e/m1-15-acceptance.spec.ts`) stops at company creation with this finding recorded rather than seeding a database row to route around it.
