...
> **Acceptance preparation updated 2026-09-03.** `e2e/m1-18-acceptance.spec.ts` exercises the real browser path through bootstrap login/password change, company creation and step-up, project creation, catalogue screen reachability, editor invitation/grant, project-local authoring, shared-password creation in project access, reader-link navigation and shared-password verification. The current flow reaches publication review but does not create a published snapshot, so the acceptance stops at the reader's explicit “Nothing published yet” state; it does not claim publication or reader-content coverage. Requirements remain In Progress/Not Started pending a deployed human run.
