...
> **Closed on 2026-08-19, with follow-up work carried forward.** Audit-log append-only enforcement and publication referential-integrity checks are implemented and tested; non-publishable pages are excluded and changelog coverage is expanded. Race-safe universal optimistic concurrency (REQ-AUTH-005) closed on 2026-08-21 — every update path now enforces the guard atomically at the repository layer. Batch atomicity and exhaustive audit/concurrency matrices remain open in REQ-FDN-025 and REQ-IMP-005.
